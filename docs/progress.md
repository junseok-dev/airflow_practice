...
- **ScrollToTop**: 페이지 이동 시 스크롤 위치 자동 초기화 (`ScrollToTop.jsx`)
- **스켈레톤 로딩**: 데이터 로드 중 shimmer 애니메이션 표시 (`Skeleton.jsx` — KPI 그리드, 차트 그리드, 테이블)
- **빈 결과 상태**: 필터 결과가 없을 때 안내 메시지 표시 (`.empty-state`)
- **CSS 전환 효과**: KPI 카드 호버 효과, 테이블 행 전환, 링크/뱃지 애니메이션
- **브라우저 탭 제목**: 페이지별 `document.title` 설정 (대시보드, 교육생 목록, 상세, 프로그램 비교)
- **필터 로딩 피드백**: 필터 변경 시 테이블 dimming (`.table-wrap--refreshing`)

### 22. 교육생 목록 기능 강화

- **지표별 컬럼 정렬**: 평가·참여·성실도·종합·활동일·클릭 헤더 클릭으로 오름차순/내림차순 토글 (`sort_by`, `sort_order` 파라미터)
- **페이지 점프 페이지네이션**: `«` / `-100` / `-50` / `-10` / `이전` / `다음` / `+10` / `+50` / `+100` / `»` 버튼

백엔드 `/api/students`에 `sort_by`, `sort_order` 쿼리 파라미터 추가.

### 23. 프로그램 비교 페이지 구현 (`/programs`)

- 6개 지표 차트 (3×2 그리드): 합격률, 이탈률 vs 고위험 비율, 종합 역량, 평가, 참여, 성실도
- 각 차트는 해당 지표 기준 정렬 + 프로그램 고유 색상 + 값 라벨 표시
- 표본 크기 vs 합격률 산점도 (점 크기 = 평균 역량 반영)
- 프로그램 종합 순위표 (정렬 기준 선택 가능)
- 프로그램 색상 범례 (AAA~GGG 각각 고유 색상)

백엔드 `GET /api/programs` 엔드포인트 추가.

### 24. 버그 수정 및 캐시 개선

- **고위험 비율 0% 버그**: `lru_cache` 영구 캐시가 ETL 이전 데이터를 유지하는 문제. `@lru_cache` → TTL 딕셔너리 캐시(10분)로 교체해 ETL 후 자동 반영되도록 수정.
- **위험 교육생 정렬 개선**: 기존 파일 저장 순(알파벳순) → 활동 기록 유무 우선 + 역량 점수 오름차순. 실제 개입이 필요한 학생이 상단에 노출되도록 개선.

### 25. 마무리 작업

- **코드 정리**: 중복 파일(`App (1).jsx`) 삭제
- **Docker 구성**: `backend/Dockerfile`, `backend/requirements.txt` 추가. `docker-compose.yaml`에 FastAPI 서비스 추가 (`docker compose up`으로 Airflow + FastAPI 동시 실행 가능)
- **README 업데이트**: 실행 명령어, 구현된 화면, API 목록, Docker 실행 방법 최신화
- **API 명세 업데이트**: 신규 엔드포인트, 정렬 파라미터, TTL 캐시 내용 반영
